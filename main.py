...
"""
    ...
    async def run_scan(self, domain: str, args) -> Dict[str, Any]:
        ...
        try:
            ...
            # Handle scoring engine import with error handling
            try:
                ...
                class SimpleScoringEngine:
                    async def score_subdomains_batch(self, subdomains, max_concurrent=10):
                        return [{'subdomain': sub, 'score': 50, 'priority': 'MEDIUM', 'breakdown': {}, 'fingerprint': ''} for sub in subdomains[:10]]
                    def get_top_scorers(self, scored_domains, top_n=20):
                        return scored_domains[:top_n]
                    def print_scoring_summary(self, scored_domains):
                        console.print(f"Scored {len(scored_domains)} domains", style="info")
        ...
        try:
            ...
            # Initialize smart scoring engine with error handling
            try:
                scoring_engine = ScoringEngine()
                console.print("✅ Smart Scoring Engine initialized", style="green")
            except Exception as e:
                ...
                class PatchedScoringEngine:
                    def __init__(self):
                        ...
                        self.total_scored = 0
                        self.performance_metrics = {'avg_scoring_time': 0, 'success_rate': 0, 'cache_hits': 0}
    ...
    async def _run_phase_subdomain_discovery(self, args, dm, results, sd, scoring_engine):
        ...
        try:
            ...
            # Score and prioritize subdomains
            if subs:
                console.print(f"    🧠 Scoring {len(subs)} subdomains...", style="blue")
                ...
                # Extract subdomains from scored results
                results["priority_targets"] = [target['subdomain'] for target in priority_targets]
                results["scored_domains"] = scored_domains  # Store full scoring data
                
                self.performance_stats['smart_targets_identified'] = len(priority_targets)

                console.print(f"    🎯 Identified {len(priority_targets)} high-value targets", style="green")
    ...
    def _print_performance_stats(self, results, leak_detector, http_client, scoring_engine):
        ...
        if hasattr(scoring_engine, 'performance_metrics'):
            ...
            console.print(f"    ⚡ Scoring performance: {metrics.get('avg_scoring_time', 0):.2f}s avg, {metrics.get('success_rate', 0)*100:.1f}% success", style="blue")
    ...
    def _initialize_results(self, domain: str, args, ai_enabled: bool) -> Dict[str, Any]:
        ...
        return {
            "subdomains": [], "urls": [], "js_files": [], "leaks": [],
            "fuzzed_paths": [], "admin_panels": [], "priority_targets": [],
            "scored_domains": [],  # New field for scoring data
            "scan_config": {
                "domain": domain, "aggressive": args.aggressive,
                "subfinder": args.subfinder, "threads": args.threads,
    ...
    def print_summary(self, results: Dict[str, Any], scan_duration: str):
        ...
        stats = {
            ...
            'scored_domains': len(results.get('scored_domains', [])),
